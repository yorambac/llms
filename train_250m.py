"""
0.25B GPT pretraining on FineWeb.
Config: n_embd=1024, n_layer=16, n_head=16, batch=8, ctx=1024, lr=1e-3

Usage:
  python train_250m.py
  python train_250m.py --resume   # auto-resumes from latest checkpoint

Checkpoints: checkpoints/run_250m/step_XXXXXX.pt  (every 1000 steps)
Metrics:     results/run_250m.csv
"""
...
def load_latest_checkpoint(ckpt_dir, model, optimizer, device):
    ...
    raw = model.module if hasattr(model, "module") else model
    # strip _orig_mod. prefix from checkpoints saved by compiled models
...
def main():
    ...
    model = GPT().to(device)
    console.print(f"[bold cyan]0.25B GPT — {model.num_params()/1e6:.1f}M params[/]")
    ...
    # advance data position to match resumed step
    if start_step > 0:
    ...
    with open(RESULTS_FILE, csv_mode, newline="") as csvfile:
        ...
        with Progress(
            ...
            task = progress.add_task(
                "0.25B", total=total_steps, completed=start_step,

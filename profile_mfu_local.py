"""
MFU profiling sweep for the 0.45B model on RTX 4070 (local).
Sweeps batch size to find optimal throughput.
Runs 50 warmup + 100 timed steps per config — no data needed.

Usage: python profile_mfu_local.py
...
WARMUP_STEPS = 50
TIMED_STEPS  = 100
PEAK_BF16    = 116.6e12   # RTX 4070 dense bf16 TFLOPS
...
def main():
    ...
    results_path = Path("results/mfu_profile_local.csv")
    results_path.parent.mkdir(exist_ok=True)

    arch = ARCH
    print(f"\nProfiling {arch['label']} on RTX 4070")
